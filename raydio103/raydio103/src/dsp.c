...
uint16_t  outBuf[ADC_BUFFER_LEN] = {
		/*
		512,562,612,661,708,753,796,837,
		874,908,938,964,985,1002,1014,1022,
		1024,1022,1014,1002,985,964,938,908,
...
*/
	...
	//float blockIN[DSP_BLOCK_SIZE] = {0.5f};
	//float blockOUT[DSP_BLOCK_SIZE] = {0};
/*
...
float	adcToNormalized(uint16_t input){
	...
	//float a = ((float)(input - ADC_HALF)/(float)ADC_MAX);
...
void dspProc(void){
	...
	if (!dspProcDone){
		...
		//HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, 1);
			...
			//arm_fir_fast_q31(&S2, blockIN, blockOUT, BUF_LEN/2);

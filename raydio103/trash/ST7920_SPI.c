...
void setFontMinWd(uint8_t wd) { cfont.minCharWd = wd; }
  ...
  void spiSelect(void){
	  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET);  // PUll the CS high
  }

  void spiDeselect(void){
	  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET);  // PUll the CS LOW
  }
// ----------------------------------------------------------------
void init(void)
{
...
void display(int buf)
{
  ...
  for(j=0;j<64/2;j++) {
    ...
    HAL_SPI_Transmit(&hspi1, (uint8_t*)(0xFA), 1, 2);
    for(i=0;i<16;i++) {  // 16 bytes from line #0+
      b=scr[i+j*16]; 
     //SPI.transfer(b & 0xF0);
      //SPI.transfer(b << 4);
      HAL_SPI_Transmit(&hspi1, (uint8_t*)0, 1, 2);
      HAL_SPI_Transmit(&hspi1, (uint8_t*)0, 1, 2);
...
void copy(uint8_t x16, uint8_t y, uint8_t w16, uint8_t h, uint8_t buf)
{
  ...
  for(j=0;j<h;j++) {
    ...
    for(i=0;i<w16;i++) {
      ...
      HAL_SPI_Transmit(&hspi1, (uint8_t*)(b & 0xF0), 1, 2);
      HAL_SPI_Transmit(&hspi1, (uint8_t*)(b << 4), 1, 2);
...
// ----------------------------------------------------------------
byte xstab[8] = {0xff,0x7f,0x3f,0x1f,0x0f,0x07,0x03,0x01};
byte xetab[8] = {0x80,0xc0,0xe0,0xf0,0xf8,0xfc,0xfe,0xff};
byte pattern[4] = {0xaa,0x55,0xaa,0x55};
...
// ----------------------------------------------------------------
int drawBitmap2(const uint8_t *bmp, int x, uint8_t y)
{
...
// ----------------------------------------------------------------
void printTxt2(uint8_t pos, uint16_t *signs)
{
  sendCmd(LCD_BASIC);
  sendCmd(pos);
